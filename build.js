...
function markdownToHTML(markdown, options = {}) {
  ...
  // Process footnotes
  // 1. Extract footnote definitions [^N]: text
...
// Returns { url, html } for preloading and display
function getHeroImage(title, slug, type, metadata = {}) {
  ...
  // Helper to create hero image HTML
  const makeHeroHtml = (url, alt) => `<img src="${url}" alt="${alt}" class="post-hero-image" width="1200" height="675" loading="eager" fetchpriority="high">`;

  // Check if this is a section page
  if (sectionImages[title]) {
    const url = sectionImages[title];
  ...
  // Using auto=format for WebP/AVIF, q=80 for quality, optimized dimensions
  const themeImages = {
    // Sailing & Ships
...
function generatePage(title, content, metadata = {}) {
  ...
  const isIndexPage = metadata.isIndexPage || false;
  ...
  const preloadLink = heroImage.url ? `<link rel="preload" as="image" href="${heroImage.url}" fetchpriority="high">` : '';
...
<head>
  ...
  <title>${title ? title + ' - ' : ''}The many travels of the Black Captain</title>
...
function buildKnowledgeBase(sectionName, sectionTitle, sectionDescription) {
  ...
  // Generate section index page (title is in post-header, don't duplicate here)
  const indexContent = `
...
function build() {
  ...
  // Generate archive page (title is in post-header, don't duplicate here)
